...
import { setStagedFilters } from '@/redux/slices/storeSlice';
...
const KeywordFilters = () => {
    ...
    const { stagedFilters } = useSelector((state : RootState) => state.store);
            ...
            <View className="pb-4 relative border-b border-b-border dark:border-b-dark-border">
                {
                    ...
                    ?
                       <View className="flex-1 justify-center items-center mt-6 mb-10">
                            <ActivityIndicator 
                            size={'small'} 
                            color={currentTheme === 'dark' ? primaryColor : '#000'} 
                            />
                        </View>
                    :
                    cuisines?.length > 0 
