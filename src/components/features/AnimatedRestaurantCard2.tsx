...
const MAX_SCALE_ON_SWIPE_LEFT = 1.05;

const AnimatedRestaurantImageSlider2 = ({ item }: { item: IRestaurant }) => {
          ...
          <Animated.View style={[styles.imageStrip, animatedImageContainerStyle]}>
            {imagesToDisplay.map((imgUri, index) => {
              const slideSpecificScaleStyle = useAnimatedStyle(() => {
                const centeredPosition = -currentIndex * SLIDER_CONTAINER_WIDTH;
                const halfwayOffLeftPosition = -(currentIndex + 0.5) * SLIDER_CONTAINER_WIDTH;
                const fullyOffLeftPosition = -(currentIndex + 1) * SLIDER_CONTAINER_WIDTH;
                ...
                if (translateX.value > 0 && index === currentIndex - 1) {
                  ...
                  dynamicZIndex = imageCount + 5; // Current image gets medium priority
                }
...
export default AnimatedRestaurantImageSlider2;
