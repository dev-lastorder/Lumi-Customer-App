...
import { ChatMessage, StoreChatBottomSheetProps } from '@/utils/interfaces/chat';
import { FlatList } from 'react-native-gesture-handler';
import { useSelector } from 'react-redux';
import { RootState, store } from '@/redux';
import { GET_CHAT_MESSAGES, GET_STORE_CHAT_MESSAGES } from '@/api/graphql/query/chat';
...
const StoreChatBottomSheet: React.FC<StoreChatBottomSheetProps> = ({ visible, onClose, orderId, storeId, storeName = 'Store' }) => {
  ...
  // Customer-to-store quick replies
  const quickReplies = [
    'Is my order ready?',
    ...
    return (
      <View className="absolute top-0 left-0 right-0 bottom-0 z-50 justify-center items-center bg-black/50">
        <View className="bg-white p-6 rounded-lg mx-4">
          ...
          <TouchableOpacity onPress={onClose} className="bg-blue-500 p-3 rounded-lg">
  ...
  return (
    <View className="absolute top-0 left-0 right-0 bottom-0 z-50" pointerEvents={visible ? 'auto' : 'none'}>
      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} className="flex-1 justify-end" pointerEvents="box-none">
        ...
        >
          <ChatHeader riderName={storeName} riderId={storeId} onClose={handleClose} />

          <View className="flex-1">
            {messages.length === 0 && !chatLoading ? (
              <>
                <EmptyState imageError={imageError} onImageError={handleImageError} />
                <QuickReplies quickReplies={quickReplies} onQuickReply={handleQuickReply} />
              </>
            ) : (
              <ChatMessagesList messages={messages} currentUserId={currentUser.id} flatListRef={flatListRef} />
            )}
          </View>

          <MessageInput messageText={messageText} onChangeText={setMessageText} onSendMessage={handleSendMessage} sendingMessage={sendingMessage} />
        </Animated.View>
...
export default StoreChatBottomSheet;
